...
def auth_headers(cookies):
    ...
    Args:
        cookies (list): website cookies
        example: [{name:"auth_token",value:"123456"}]
    ...
    for cookie in cookies:
        if "auth_token" == cookie["name"]:
            auth_token = cookie["value"]
        if "TYCID" == cookie["name"]:
            tycid = cookie["value"]
    assert auth_token and tycid
